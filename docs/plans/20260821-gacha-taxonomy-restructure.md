...
## Phase 3 완료 (2026-08-25) — collector

### 최종 상태 (dev)

```
시리즈        4,256  →  active 3,717 / archived 539
노출 후보(is_browsable, 임계값 4)          319
커버 상품     5,766  →  7,905 / 10,100      57.1% → 78.3%
kind unknown 비율                          2.7%   (목표 30% 이하)
계층                                       36건, depth ≤ 2
```

병합 5라운드(name_ja 483 · 이명 13 · 롱테일 42 · 수동 2 · 포켓몬 1) 동안 **상품 유실 0건**. `merged_into_id` + `refresh_gacha_product_series()` 설계가 의도대로 동작했다.

### 임계값을 4로 정한 근거

"예상 범위 200~400"이 아니라 **칩별 분포**로 정했다. 루트 시리즈 기준:

| 칩                      | ≥3  | ≥4  | ≥5  |
| ----------------------- | --- | --- | --- |
| other                   | 170 | 95  | 63  |
| 애니메이션(anime+manga) | 93  | 65  | 51  |
| 캐릭터                  | 45  | 37  | 34  |
| 프랜차이즈              | 44  | 36  | 29  |
| 제품 라인               | 43  | 32  | 27  |
| 게임                    | 31  | 23  | 17  |

4가 **모든 칩이 쓸 만한 크기를 유지하는 최대 임계값**이다. 5로 가면 게임 칩이 17까지 떨어진다.

### G13(`other` 47%)는 오분류가 아니었다

`other` 내용을 확인해보니 `애니멀 어트랙션`(41), `일하는 고양이`(32), `꼬비토즈칸`(29), `신수 베코들`(16), `산책 개구리`(13) 같은 **일본 오리지널 캡슐토이 시리즈**였다. anime/game/franchise 어디에도 안 맞는 게 정상이며 분류가 오히려 잘 된 것이다.

→ **F3 해소**: 숨기지 않고 `오리지널` 칩으로 노출한다. 기획서 §6-3 갱신.

### Phase 4 브라우징 RPC 실데이터 검증 (마지막 미검증 항목)

`is_browsable` 이 켜지면서 처음으로 실데이터 검증이 가능해졌고, 전부 통과했다.

```
browse_gacha_series()   293행 (루트만)   total_count 293
kind 필터               anime 37 · other 95
계층 롤업               산리오 캐릭터즈 388 = 자기 201 + 자식 10개 187
자손 포함 토글           388 / 미포함 201
카테고리 교차            산리오 ∩ 마스코트 44
```

293 = `is_browsable` 319 − 자식 26. 목록은 루트만 보여주므로 의도한 동작이다.

### 🔴 검증 중 발견한 결함 — 병합 체인 (내 쪽 버그, 수정 완료)

활성 상품 3건이 archived 시리즈에만 매핑되어 **시리즈 탐색에서 완전히 사라져 있었다.**

```
산리오 하피단부이 ──> 산리오 해피단부이 ──> 해피단부이
산리오 해피댄버   ──> 산리오 해피단부이 ──> 해피단부이
```

원인 두 가지 모두 내가 만든 것이다:

1. **Phase 1 트리거가 체인을 못 막는다.** `merged_into_id` 설정 시 "내 타깃이 이미 병합됐는가"(아래쪽)만 검사하고 "나를 가리키는 것이 있는가"(위쪽)는 보지 않는다. `A→B` 후 `B→C` 하면 각 단계는 통과하지만 결과적으로 체인이 된다.
2. **`refresh_gacha_product_series()` 가 한 홉만 따라간다.** `coalesce(s.merged_into_id, s.id)` 라서 중간 지점이 archived면 거기서 멈춘다.

수정: `20260825_gacha_series_merge_chain_flatten.sql`

- 기존 체인을 최종 타깃으로 평탄화
- AFTER 트리거로 앞으로도 자동 평탄화 (거부가 아니라 교정 — 배치 순서 때문에 정상 작업이 막히면 안 되므로)
- `refresh_gacha_product_series()` 재실행으로 끊긴 매핑 복구

검증: 체인 0 / archived 매핑 0 / 고아 상품 0 / 커버 7,905 유지. 롤백 트랜잭션에서 `A→B` 후 `B→C` 시 A가 자동으로 C를 가리키는 것 확인.

**교훈**: "체인 금지" 제약을 걸어놓고 한 방향만 검사했다. 무결성 제약은 양방향을 다 봐야 한다.

### 남은 것

- `name_ja` 한글 오염 B그룹 15건 (일본어 원문이 살아있어 null 처리하지 않음) → F6
- prod 미적용
...
### ~~F3. `other` 칩을 만들지 결정 필요~~ → 해소 (2026-08-25)

내용을 확인해보니 `other` 는 오분류가 아니라 일본 오리지널 캡슐토이 시리즈였다. **`오리지널` 칩으로 노출**하기로 하고 기획서 §6-3을 갱신했다. `unknown`(5건)만 `전체` 전용으로 남긴다.

### F6. `name_ja` 한글 오염 B그룹 15건

일본어 원문 안에 한글이 소수 섞인 케이스다. 문자 단위 치환 버그로 보이며 원문이 거의 살아있다.
...
null 로 비우면 복구 가능한 정보를 버리므로 이번 정리(A그룹 31건)에서 제외했다. 처리 방법은 역치환 매핑(`카→カ`, `리→リ`, `미→ミ`, `비→ビ`, `션→ション`) 자동 복구가 우선이고, 안 잡히는 건 `note` 에 review 표시만 남긴다.
